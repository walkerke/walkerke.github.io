...
erasing water area from Census tracts obtained with pygris.
...
that we need `%matplotlib inline` and to use `plt.show()`:major reason
for linking pygris with ArcGIS Pro, however, is to gain access to ArcGIS
Pro's rich cartographic functionality. This will require a couple extra
steps.
